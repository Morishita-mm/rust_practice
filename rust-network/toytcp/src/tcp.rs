...
impl TCP {
    ...
    /// タイマースレッド用の関数
    /// すべてのソケットの再送キューを見て、タイムアウトしているパケットを再送する
    fn timer(&self) {
        ...
        loop {
            ...
            for (sock_id, socket) in table.iter_mut() {
                while let Some(mut item) = socket.retransmission_queue.pop_front() {
                    ...
                    {
                        // 取り出したエントリがタイムアウトしていないなら、キューの以降のエントリもタイムアウトしていない
                        // 先頭に戻す
    ...
    pub fn connect(&self, addr: Ipv4Addr, port: u16) -> Result<SockID> {
        ...
        // 受信スレッドがロックを取得できるようにするためにロックを外してイベントを待機
        drop(table);
    ...
    fn syssent_handler(&self, socket: &mut Socket, packet: &TCPPacket) -> Result<()> {
        dbg!("sysent handler");
        if packet.get_flag() & tcpflags::ACK > 0
            && socket.send_param.unacked_seq <= packet.get_ack()
            && packet.get_ack() <= socket.send_param.next
    ...
    /// バッファーのデータを送信する。必要であれば複数のバケットに分割して送信する
    /// すべて送信したら(まだackされていなくても)リターンする
    ...
    pub fn close(&self, sock_id: SockID) -> Result<()> {
        ...
        match socket.status {
            TcpStatus::Established => {
                socket.status = TcpStatus::FinWait1;
                // ロックを外してイベントの待機、樹脂スレッドがロックを取得できるようにする
                drop(table);
