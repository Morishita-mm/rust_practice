...
fn main() {
    ...
    // ヒープ常にDhcpServer構造体を確保し、複数のスレッドから共有するためArcを利用する
...
fn dhcp_request_message_handler_responded_to_offer(
    ...
    // DHCPOFFERメッセージに対する応答の場合、必ず'requested IP
    // address'に割り当て予定のIPアドレスが含まれる
...
fn dhcp_request_message_handler_to_reallocate(
    ...
    if let Some(requested_ip) = received_packet.get_option(Code::RequestedIpAddress as u8) {
        debug!("client is in IIT-REBOOT");
        // クライアントが以前割り当てられたIPアドレスを記憶しており、再起動状態にある時
        let requested_ip = util::u8_to_ipv4addr(&requested_ip)
            .ok_or_else(|| failure::err_msg("Failed to convert ip adr."))?;
        ...
        match database::select_entry(&con, client_macaddr)? {
            Some(ip) => {
                if ip == requested_ip && dhcp_server.network_addr.contains(ip) {
                    // 以前割り当てたIPアドレスと要求されたIPアドレスが一致しており、ネットワークに含まれているときはACKを返す
...
fn dhcp_release_message_handler(
    ...
    // DHCPOFFERメッセージを返す際に解放済みのIPアドレスを再割り当てする場合があるため物理削除ではなく論理削除
...
fn select_lease_ip(
    ...
    Err(failure::err_msg("Counld not obtain available ip address."))
}
